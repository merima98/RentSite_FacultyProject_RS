...
{
    public class RentedRoomsService : BaseService<Model.RentedRooms, RoomSearchRequest, Database.RentedRooms>
    {
        ...
        public override List<Model.RentedRooms> GetAll(RoomSearchRequest search)
        {
                ...
                foreach (var room in listRooms)
                {
                    foreach (var user in listUser)
                    {
                        ...
                        if (rented.EndRentalDate == DateTime.Parse("9999-12-31") && rented.UserId == user.Id && rented.RoomId==room.Id && room.Rented ==true)
                        {
                            ...
                            if (search?.TypeOfRoomId.HasValue == false && search?.CityId.HasValue == false)
                            {
                                Model.RentedRooms temp = new Model.RentedRooms();
                                temp.Id = rented.Id;
                                temp.LastName = user.LastName;
